...
export function cloud() {
  ...
  function generateInnerClouds(cloudWidth, cloudHeight) {
    ...
    for (var i = 0; i < 15; i++) {
      var x = random(40, 40 + cloudWidth)
      var y = random(40, 40 + cloudHeight)
      innerClouds.push([
        x,
        y,
        random(0, 360),
  ...
  function getCloudCanvas(frame) {
    if (!canvases[frame]) {
      ...
      clouds.forEach(el => drawCloud(el, img, newContext));
      innerClouds.forEach(el => drawCloud(el, imgInner, newContext));
